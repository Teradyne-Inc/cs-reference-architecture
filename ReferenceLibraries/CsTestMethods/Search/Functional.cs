...
namespace CsTestMethods.Search {

    [TestClass]
    public class Functional : TestCodeBase {

        private const double _notFoundResult = double.NaN;
        ...
        public void LinearStop(Pattern pattern, string forcePins, double from, double to, int count, double waitTime) {
            ...
            for (int i = 0; i < count; i++) {
                ...
                ForEachSite(site => {
                    if (double.IsNaN(results[site]) && value[site]) results[site] = inValue;
                });
                inValue += increment;
                if (results.All(s => !double.IsNaN(s))) break;
            }
