...
namespace Csra.Setting.TheHdw.Digital.Pins.Levels {
    ...
    public class Value_Voh : Setting_double {
        ...
        internal Value_Voh(string value, string pinList) : this(double.Parse(value), pinList) { }
        ...
        protected override void SetAction(string pinList, double value) {
            TestCodeBase.TheHdw.Digital.Pins(pinList).Levels.Value[ChPinLevel.Voh] = value;
        }

        protected override double[] ReadFunc(string pin) {
            double[] result = new double[TheExec.Sites.Existing.Count];
            ForEachSite(site => result[site] = TestCodeBase.TheHdw.Digital.Pins(pin).Levels.Value[ChPinLevel.Voh]);
            return result;
        }
