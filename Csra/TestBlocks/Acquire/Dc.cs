...
namespace Csra.TheLib.Acquire {
    ...
    public class Dc : ILib.IAcquire.IDc {
        ...
        public virtual PinSite<Samples<double>> MeasureSamples(Pins pins, int sampleSize, double? sampleRate = null, DcMeterMode? meterMode = null) {
            ...
            if (pins.ContainsFeature(InstrumentFeature.Ppmu)) resultPin.Add(pins.Ppmu.MeasureSamples(sampleSize));
            if (pins.ContainsFeature(InstrumentFeature.Dcvi)) {
        ...
        public virtual PinSite<Samples<double>> ReadCapturedSamples(Pins pins, string signalName) {
            List<PinSite<Samples<double>>> resultPin = new();
            if (pins.Dcvi != null) {
                resultPin.Add(pins.Dcvi.ReadSignal(signalName));
            }
            if (pins.Dcvs != null) {
                resultPin.Add(pins.Dcvs.ReadSignal(signalName));
            }
            return pins.ArrangePinSite(resultPin);
        }
        ...
        public virtual PinSite<double> ReadMeasured(Pins pins, int sampleSize, double? sampleRate = null) {
            List<PinSite<double>> resultPin = new();
            if (pins.ContainsFeature(InstrumentFeature.Ppmu)) resultPin.Add(pins.Ppmu.Read());
            if (pins.ContainsFeature(InstrumentFeature.Dcvi)) {
        ...
        public virtual PinSite<Samples<double>> ReadMeasuredSamples(Pins pins, int sampleSize, double? sampleRate = null) {
            ...
            if (pins.ContainsFeature(InstrumentFeature.Ppmu)) {
                Api.Services.Alert.Error($"PPMU pin does not support {nameof(ReadMeasuredSamples)} option.");
            }
            if (pins.ContainsFeature(InstrumentFeature.Dcvi)) {
        ...
        public virtual void StrobeSamples(Pins pins, int sampleSize, double? sampleRate = null) {
            ...
            if (pins.Ppmu != null) {
                Api.Services.Alert.Warning($"PPMU pin does not support {nameof(StrobeSamples)} option.");
            }
