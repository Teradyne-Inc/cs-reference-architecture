...
namespace Csra.Setting.TheHdw.Digital.Pins.Levels {
    ...
    public class Value_Vt : Setting_double {

        private static readonly Dictionary<string, double> _staticCache = [];

        internal Value_Vt(string value, string pinList) : this(double.Parse(value), pinList) { }
        ...
        private void SetAction(string pinList, double value) {
            TestCodeBase.TheHdw.Digital.Pins(pinList).Levels.Value[ChPinLevel.Vt] = value;
        }

        private double[] ReadFunc(string pin) {
            double[] result = new double[TheExec.Sites.Existing.Count];
            ForEachSite(site => result[site] = TestCodeBase.TheHdw.Digital.Pins(pin).Levels.Value[ChPinLevel.Vt]);
            return result;
        }
