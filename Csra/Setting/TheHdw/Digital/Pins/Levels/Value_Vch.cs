...
namespace Csra.Setting.TheHdw.Digital.Pins.Levels {
    ...
    public class Value_Vch : Setting_double {
        ...
        internal Value_Vch(string value, string pinList) : this(double.Parse(value), pinList) { }
        ...
        private void SetAction(string pinList, double value) {
            TestCodeBase.TheHdw.Digital.Pins(pinList).Levels.Value[ChPinLevel.Vch] = value;
        }

        private double[] ReadFunc(string pin) {
            double[] result = new double[TheExec.Sites.Existing.Count];
            ForEachSite(site => result[site] = TestCodeBase.TheHdw.Digital.Pins(pin).Levels.Value[ChPinLevel.Vch]);
            return result;
        }
