...
namespace Csra.Setting.TheHdw.Dcvs.Pins.CurrentLimit.Sink.OverloadLimit {
    ...
    public class Level : Setting_double {
        ...
        private static void SetAction(string pinList, double value) {
            TestCodeBase.TheHdw.DCVS.Pins(pinList).CurrentLimit.Sink.OverloadLimit.Level.Value = value;
        }

        private static double[] ReadFunc(string pin) {
            double[] result = new double[TheExec.Sites.Existing.Count];
            ForEachSite(site => result[site] = TestCodeBase.TheHdw.DCVS.Pins(pin).CurrentLimit.Sink.OverloadLimit.Level.Value);
            return result;
        }
