...
namespace CsraTestMethods.Trim {
    ...
    public class Digital : TestCodeBase {
        ...
        private Site<int> _valuesBinary;
        ...
        private const int _notFoundResult = int.MinValue;
        ...
        public void LinearStopTrip(Pattern pattern, int from, int to, int count, string setup = "") {
            ...
            if (ShouldRunBody) {
                _values = TheLib.Acquire.Search.LinearStopFromToCount(from, to, count, 0, _notFoundResult, (modIndex) => {
                    TheHdw.Patterns(_pattern[0].Name + $":mod{modIndex}").Start();
        ...
        public void LinearStopTarget(Pattern pattern, PinList capPins, int from, int to, int count, int target, string setup = "") {
            ...
            if (ShouldRunBody) {
                ...
                _values = TheLib.Acquire.Search.LinearStopFromToCount(from, to, count, 0, _notFoundResult, (modIndex) => {
                    ...
                    TheHdw.Patterns(_pattern[0].Name + $":hram_mod{modIndex}").Start();
                    TheHdw.Digital.Patgen.HaltWait();
        ...
        #region BinaryTrip
        [TestMethod, Steppable, CustomValidation]
        public void BinaryTrip(Pattern pattern, int from, int to, int minDelta, bool invertedOutput, string setup = "") {
            ...
            if (ShouldRunPreBody) {
                ...
                for (int i = 0; i < _moduleCount; i++) {
                    string name = _pattern[0].Name + $":mod{i}";
            ...
            if (ShouldRunBody) {
                ...
                _valuesBinary = TheLib.Acquire.Search.BinarySearch(from, to, minDelta, invertedOutput, (modIndex) => {
                    ForEachSite(site => {
                        patSpec[site] = _pattern[0].Name + $":mod{modIndex[site]}";
                    });
        ...
        #region BinaryTarget
        [TestMethod, Steppable, CustomValidation]
        public void BinaryTarget(Pattern pattern, PinList capPins, int from, int to, int minDelta, bool invertedOutput, int target, string setup = "") {
            ...
            if (ShouldRunPreBody) {
                ...
                Services.Setup.Apply(setup);
                for (int i = 0; i < _moduleCount; i++) {
                    string name = _pattern[0].Name + $":hram_mod{i}";
                    PatternInfo patternTemp = new PatternInfo(name, true);
                }
            }
            ...
            if (ShouldRunBody) {
                ...
                _valuesBinary = TheLib.Acquire.Search.BinarySearch(from, to, minDelta, invertedOutput, (modIndex) => {
                    ...
                    ForEachSite(site => {
                        patSpec[site] = _pattern[0].Name + $":hram_mod{modIndex[site]}";
                    });
                    TheHdw.PatternsPerSite(patSpec).Start();
                    TheHdw.PatternsPerSite(patSpec).HaltWait();
                    _readWords = TheLib.Acquire.Digital.ReadWords(_pins, 0, 8, 8, tlBitOrder.LsbFirst);
                    ForEachSite(site => {
                        result[site] = true ? modIndex[site] : _readWords[0][site][0]; // Simulate data process
                    });
                    return result;
                },
                target
