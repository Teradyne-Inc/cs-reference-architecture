...
namespace Csra.Setting.TheHdw.Digital.Pins.Levels {
    ...
    [Serializable]
    public class Value_Iol : Setting_double {

        private static readonly Dictionary<string, double> _staticCache = [];

        internal Value_Iol(string value, string pinList) : this(double.Parse(value), pinList) { }

        public Value_Iol(double value, string pinList) {
            SetArguments(value, pinList, true);
            SetBehavior(0, "A", SetupResetTrigger.OnProgramStarted, true);
            SetContext(true, _staticCache);
            if (TheExec.JobIsValid) Validate();
        }

        protected override void SetAction(string pinList, double value) {
            TestCodeBase.TheHdw.Digital.Pins(pinList).Levels.Value[ChPinLevel.Iol] = value;
        }

        protected override double[] ReadFunc(string pin) {
            double[] result = new double[TheExec.Sites.Existing.Count];
            ForEachSite(site => result[site] = TestCodeBase.TheHdw.Digital.Pins(pin).Levels.Value[ChPinLevel.Iol]);
            return result;
        }
