...
namespace Csra.Types {
    ...
    public class PinsFactory {
        ...
        public static List<string> GetPins(string pinList, out IPpmuPins ppmuPins, out IDcviPins dcviPins, out IDcvsPins dcvsPins, out IDigitalPins digitalPins, out IUtilityPins utilityPins) {
            ...
            // The csvPins.Distinct() above only removes duplicate pin-group tokens, not the individual pins they decompose to.
            // When overlapping groups are supplied (e.g. one group is a subset of another), the same physical pin is added
            // multiple times, which later surfaces as a "pins are duplicated" error during measurement. Deduplicate here while
            // preserving first-occurrence order so pin sequencing (used by ArrangePinSite) remains stable.
