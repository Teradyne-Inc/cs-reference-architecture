    ...
    public class Dc : ILib.ISetup.IDc {
        ...
        public virtual void SetForceAndMeter(Pins pins, DcOutputMode mode, double forceValue, double forceRange, double clampValue, DcMeterMode meterMode, double measureRange, bool gateOn = true) {
            if (mode == DcOutputMode.ForceCurrent) {
                ...
                if (pins.ContainsFeature(InstrumentFeature.Dcvs) && Math.Abs(forceValue) > forceRange) {
                    Api.Services.Alert.Warning($"SetForceAndMeter: |forceValue| ({Math.Abs(forceValue)}V) exceeds forceRange ({forceRange}V) on a DCVS pin. " +
                        "DCVS validates the forced voltage against forceRange and will raise DCVS:0003. Increase forceRange to cover forceValue.");
                }
                if (meterMode == DcMeterMode.Current) {
                    ...
                    pins.Dcvs?.ForceVSetMeterI(forceValue, clampValue, measureRange, gate: gateOn ? true : null);
                } else { // voltage
                    // PPMU has no independent voltage meter range: ForceVMeasureV's range argument is a current range, not a voltage range.
                    // Use clampValue (the force current clamp), not measureRange (a voltage meter range) which would select a nonsensical PPMU current range.
                    pins.Ppmu?.ForceVSetMeterV(forceValue, clampValue, gateOn ? true : null);
                    pins.Dcvi?.ForceVSetMeterV(forceValue, clampValue, measureRange, gateOn ? true : null);
                    // DCVS has no independent meter voltage range: VoltageRange is the force range. Use forceRange (not measureRange) to avoid DCVS:0003 when forceValue > measureRange.
                    pins.Dcvs?.ForceVSetMeterV(forceValue, clampValue, forceRange, gateOn ? true : null);
                }
            } else { // HiZ
                pins.Ppmu?.ForceHiZ();
                pins.Dcvi?.ForceHiZ();
