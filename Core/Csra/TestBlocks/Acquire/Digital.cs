...
namespace Csra.TheLib.Acquire {
    ...
    public class Digital : ILib.IAcquire.IDigital {
        ...
        public virtual bool TryReadFailsCapture(Tol.IDigitalCapture digitalCapture, out PinSite<int[]> fails) {
            try {
                fails = digitalCapture.ReadFails();
                return true;
            }
            catch (Exception ex) {
                fails = null;
                Api.Services.Alert.Error($"Failed to read fails capture: {ex.Message}");
                return false;
            }
        }

        public virtual bool TryReadStvCapture(Tol.IDigitalCapture digitalCapture, out PinSite<double[]> stv) {
            try {
                stv = digitalCapture.ReadStv();
                return true;
            }
            catch (Exception ex) {
                stv = null;
                Api.Services.Alert.Error($"Failed to read STV capture: {ex.Message}");
                return false;
            }
        }
    }
