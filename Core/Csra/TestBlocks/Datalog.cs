...
namespace Csra.TheLib {
    public class Datalog : ILib.IDatalog {
        ...
        public virtual void TestFunctional(Site<bool> result, string pattern = "", string tName = "") => TheExec.Flow.FunctionalTestLimit(result, pattern, TName: tName);
        ...
        public virtual void TestParametric(PinSite<Samples<double>> result, double forceValue = 0, string forceUnit = "", bool sameLimitForAllSamples = false) {
            if (OfflinePassResults) {
                result.Fill(new Samples<double>(GetSampleCount(result), MidWayBetweenLimits()), tlSiteType.Existing);
            }
            TheExec.Flow.TestLimit(result, CompareMode: tlLimitCompareType.EachSample, ForceVal: forceValue, ForceUnit: UnitType.Custom, CustomForceunit: forceUnit,
        ...
        public virtual void TestParametric(Site<int> result, double loVal, double hiVal, string tName = "") {
            if (OfflinePassResults) {
                result.Fill((int)((loVal + hiVal) / 2.0), tlSiteType.Existing);
            }
        ...
        public virtual void TestParametric(PinSite<double> result, double loVal, double hiVal, string tName = "") {
            if (OfflinePassResults) {
                result = result.Select2d(_ => (loVal + hiVal) / 2.0);
            }
            TheExec.Flow.TestLimit(result, loVal, hiVal, TName: tName, ForceResults: tlLimitForceResults.None);
        }
