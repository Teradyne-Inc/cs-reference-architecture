    ...
    public class Alert : IAlertService {
        ...
        public void Error<TException>(string error, int validationArgumentIndex = 0, [CallerMemberName] string doNotSpecify = "") where TException :
            ...
            if (TheExec.Flow.IsValidating) {
                try {
                    TheExec.DataManager.WriteTemplateArgumentError(validationArgumentIndex, 0, error, "C#RA");
                    return;
                } catch (Exception ex) when (IsNoCurrentTestInstance(ex)) {
                    // No current test instance is available (for example when called from a program-level
                    // Exec Interpose such as OnValidationStart). Fall through to log + throw so the real
                    // error is preserved instead of surfacing IG-XL's "Mangled test instance name" error.
                    Log($"WriteTemplateArgumentError unavailable outside a test instance context: {ex.Message}");
                }
            }
        ...
        // Narrows the catch above to the specific "no current test instance" failure so that any other,
        // legitimate exception from WriteTemplateArgumentError (e.g. a bad validationArgumentIndex, or an
        // unrelated IG-XL internal state issue) still propagates instead of being silently swallowed.
        private static bool IsNoCurrentTestInstance(Exception ex) {
            return ex.Message.IndexOf("Mangled test instance name", StringComparison.OrdinalIgnoreCase) >= 0;
        }
