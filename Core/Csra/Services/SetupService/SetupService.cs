...
namespace Csra.Services {
    ...
    public class Setup : ISetupService {
        ...
        public void Apply(string setupNames) {
            ...
            foreach (string setup in split) {
                string name = setup.Trim();
                if (!_setups.TryGetValue(name, out Csra.Setup target)) {
                    Api.Services.Alert.Error<KeyNotFoundException>(
                        $"Setup '{name}' is not registered. {_setups.Count} setup(s) currently registered. " +
                        "Ensure the setup is registered (e.g. in an ExecInterpose hook) before it is applied.");
                    continue;
                }
                target.Apply();
            }
